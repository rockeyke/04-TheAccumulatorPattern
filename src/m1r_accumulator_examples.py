...
#
#              for k in range(r - s): --> should be s-r not r-s
#                  count = 0 --> should be assigned outside the for loop
#                  if math.cos(r) > 0: --> does not include 0
#                      count = 1 --> should be count = count+1 or else count will always be 1
#                  return count --> should be in line with the for loop not the if statement. Terminates early
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean? --> it is the iteration of the loop. The counter for how many times the
#                                                   loop has run
#
...
#                  circle = rg.Circle(point, radius)
#                  circle.attach_to(window)
#                  x = x + (2 * radius) --> this one is wrong
#
###############################################################################
